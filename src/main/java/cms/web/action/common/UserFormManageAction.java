...
public class UserFormManageAction {
    ...
    private void setSession(HttpServletRequest request, HttpServletResponse response, User user) {

        List<UserRole> userRoleList = userRoleService.findRoleByUserName(user.getUserName());
        boolean downloadAccess = false;
        if (!CollectionUtils.isEmpty(userRoleList)) {
            A:
            for (UserRole role : userRoleList) {
                List<UserResourceGroup> userResourceGroupList = JsonUtils.toGenericObject(role.getUserResourceFormat(), new TypeReference<List<UserResourceGroup>>() {
                });

                for (UserResourceGroup userResourceGroup : userResourceGroupList) {
                    if (userResourceGroup.getCode() == 7000000 && userResourceGroup.getSelected()) {
                        downloadAccess = userResourceGroup.getUserResourceList().get(0).getSelected();
                        break A;
                    }
                }
            }
        ...
        request.getSession().setAttribute("downloadAccess", downloadAccess);
//        request.setAttribute("downloadAccess", downloadAccess);
    }
